...
let constructor = function(app) {
    return {
        ...
        existsSubtree: function(package_name) {
            ...
            return (
                app.utils.shell.call(
                    'find . -type d -wholename "./' + directory + '"',
                    app.getPath(),
                    true
                ).stdout !== ''
            );
        ...
        addSubtree: function(package_name, repository, silent = true) {
            ...
            let directory = app.utils.package.getDirectoryByName(package_name);
            let response = app.utils.shell.call(
                'git remote add ' +
                    directory +
                    ' ' +
                    repository +
        ...
        pullSubtree: function(package_name, repository, silent = true) {
            ...
            let directory = app.utils.package.getDirectoryByName(package_name);
            let response = app.utils.shell.call(
                'git subtree pull --prefix=' + directory + '/ ' + repository + ' master --squash',
                app.getPath(),
                silent
            );
            if (response.code !== 0 && !silent) {
                app.utils.ui.error(
                    'Error pulling the package [' +
                        package_name +
                        '] subtree from [' +
                        repository +
                        '].'
        ...
        commitChanges: function(silent = true) {
            ...
            return app.utils.ui.ask('Commit message:', 'WIP').then(message => {
