...
let constructor = function (app) {
    ...
    return app.abstracts.command.extend({
        name: 'git:subtree:push [package-name]',
        ...
        action: function (package_name, cmd) {
            ...
            getPackageName(package_name)
                    ...
                    .then(result => {
                        if (!result) {
                            if (!verbosity) {
                                app.utils.ui.error('Error pushing the packages subtrees. Cannot commit current changes!');
                            }
                            this.exit(-1);
                        }
                        ...
                        packages.forEach(data => {
                            ...
                            let done = app.utils.git.pushSubtree(data[0], data[1], !verbosity);
                            if (!verbosity) {
