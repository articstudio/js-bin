...
let constructor = function(app) {
    ...
    let getPackageName = function(package_name) {
        return package_name
            ? app.utils.promised(package_name)
            : app.utils.ui.selectPackage(false, [{ name: 'New Package', value: 'new' }]);
    };
    let getNewPackageName = function(package_name) {
        return package_name === 'new'
            ? app.utils.ui.ask('Please enter the name of the package:')
            : app.utils.promised(package_name);
    };
    let getPackageRepository = function(repository) {
        return repository
            ? app.utils.promised(repository)
            : app.utils.ui.ask('Please enter the URL of the git repository:');
    };
    let getSavePackage = function(save) {
        return save !== null
            ? app.utils.promised(save)
            : app.utils.ui.confirm('Store this package/repository to the package.json?', true);
    };
                    ...
                    if (!result) {
                        if (!verbosity) {
                            app.utils.ui.error(
                                'Error adding the package [' +
                                    package_name +
                                    '] subtree. Cannot commit current changes!'
                            );
                        }
                        this.exit(-1);
                    }
