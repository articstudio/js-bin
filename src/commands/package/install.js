...
let constructor = function(app) {
    ...
    let getNewPackageName = function(package_name) {
        return !package_name
            ? app.utils.ui.ask('Please enter the name of the package (<package-name>[:<version>]):')
            : app.utils.promised(package_name);
    };
    let getSaveDevPackage = function(save) {
        return save !== null
            ? app.utils.promised(save)
            : app.utils.ui.confirm('Save this package in devDependencies?', true);
    };
    ...
    return app.abstracts.command.extend({
        ...
        action: function(package_name, target_package_name, cmd) {
            ...
            let save_dev =
                !!cmd.saveDev === cmd.saveDev ? true : !!cmd.save === cmd.save ? false : null;
            let version = null;
                    ...
                    if (!app.utils.package.checkExistsByName(target_package_name)) {
                        app.utils.ui.error(
                            'Error install the package [' +
                                ...
                                '] to [' +
                                target_package_name +
                                ']. Target package.json not exists!'
                        );
                        this.exit(-1);
                    }
                        ...
                        if (!verbosity) {
                            app.utils.ui.error(
                                'Error installing the package [' +
                                    app.utils.package.getPackageFullname(package_name, version) +
                                    '].'
                            );
                        }
                        this.exit(-1);
